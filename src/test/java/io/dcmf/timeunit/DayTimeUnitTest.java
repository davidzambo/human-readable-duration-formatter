...
class DayTimeUnitTest {
    DayTimeUnit timeUnit;
    private static final int TIME_UNIT_STEP = 60 * 60 * 24;

    @BeforeEach
    void setUp() {
        this.timeUnit = new DayTimeUnit();
    }
    ...
    void getConvertedTimeUnitWithOneValue() {
        this.timeUnit.setTimeValue(TIME_UNIT_STEP);
        assertEquals(this.timeUnit.getConverterTimeUnit(), "1 day");
    }

    @Test
    void getConvertedTimeUnitWithMultipleValue() {
        this.timeUnit.setTimeValue(TIME_UNIT_STEP * 2);
        assertEquals(this.timeUnit.getConverterTimeUnit(), "2 days");
    }

    @Test
    void getConvertedTimeUnitWithLessThanOneValue() {
        this.timeUnit.setTimeValue(TIME_UNIT_STEP / 2);
        assertEquals(this.timeUnit.getConverterTimeUnit(), "0 day");
    }
}