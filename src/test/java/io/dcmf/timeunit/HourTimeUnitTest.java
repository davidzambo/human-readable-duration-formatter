...
class HourTimeUnitTest {
    HourTimeUnit timeUnit;
    private static final int TIME_UNIT_STEP = 60 * 60;

    @BeforeEach
    void setUp() {
        this.timeUnit = new HourTimeUnit();
    }
    ...
    void getConvertedTimeUnitWithOneValue() {
        this.timeUnit.setTimeValue(TIME_UNIT_STEP);
        assertEquals(this.timeUnit.getConverterTimeUnit(), "1 hour");
    }

    @Test
    void getConvertedTimeUnitWithMultipleValue() {
        this.timeUnit.setTimeValue(TIME_UNIT_STEP * 2);
        assertEquals(this.timeUnit.getConverterTimeUnit(), "2 hours");
    }

    @Test
    void getConvertedTimeUnitWithLessThanOneValue() {
        this.timeUnit.setTimeValue(TIME_UNIT_STEP / 2);
        assertEquals(this.timeUnit.getConverterTimeUnit(), "0 hour");
    }
}